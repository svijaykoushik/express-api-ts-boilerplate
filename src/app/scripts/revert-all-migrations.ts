...
async function run() {
    if (await confirmAction()) {
        ...
        for (const migration of migrations) {
            ...
            count++;
            console.log(`✳️ Reverted ${count}/${migrations.length} ${migration.name}`);
        }
        return true;
    } else {
        return false;
...
async function confirmAction() {
    return new Promise<boolean>((resolve, reject) => {
        try {
            ...
            rl.question('‼️ Reverting all migrations can cause issues in the applicaion. Are you sure? (y/n)', (response) => {
                if (
